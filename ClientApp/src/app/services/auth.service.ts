...
export class AuthService {
    ...
    login(userName: string, password: string, rememberMe?: boolean) {
        ...
        let user = new User("123", "admin", "admin", "admin@gmail.com", "job", "123456789");
        this.saveUserDetails(user, [], "abc", "abc", "abc", new Date(), rememberMe);
        let response = { access_token: "123", id_token: "123", refresh_token: "123", expires_in: new Date() };
        
        let res = new Observable < LoginResponse >();
        return res.map(response => user);
